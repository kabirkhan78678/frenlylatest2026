...
export async function getAllMessages(req, res) {
    try {
        ...
        const messages = await prisma.chatMessage.findMany({ where: { chatId: parseInt(chatId) }, include: { sender: true, chat: true }, orderBy: { createdAt: 'desc' } });
        const unreadCountData = await prisma.unreadCount.findFirst({ where: { userId: req.user.id, chatId: parseInt(chatId) } });
